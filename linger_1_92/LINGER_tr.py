...
def get_TSS(GRNdir,genome,TSS_dis,outdir):
    #import pyensembl
...
# Retrieve TSS positions for each gene and store them in a list
    ...
    Tssdf[['chr','1M-','1M+','symbol','TSS', 'strand']].to_csv(f'{outdir}TSS_extend_1M.txt',sep='\t',index=None)
...
def sc_nn_NN(ii,RE_TGlink_temp,Target,Exp,Opn,l1_lambda,activef):
    ...
    for i in range(100):
        ...
        optimizer.step()
    np.random.seed(42)
    num_samples = min(50, X_tr.shape[0])  # Choose the smaller value between 50 and the number of samples
    background = X_tr[np.random.choice(X_tr.shape[0], num_samples, replace=False)]
