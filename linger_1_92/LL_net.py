...
#Map row and col indices for lookup
    ...
    row_indices = np.array([row_map[row] for row in df[i_n]])
    col_indices = np.array([col_map[col] for col in df[j_n]])
    matrix = csr_matrix((df[x_n], (row_indices, col_indices)), shape=(len(REs), len(TFs)))
...
def load_TFbinding(GRNdir,O_overlap,O_overlap_u,O_overlap_hg19_u,chrN):
    TFbinding=pd.read_csv(os.path.join(GRNdir, f'TF_binding_{chrN}.txt'),sep='\t',index_col=0)
...
def load_region(GRNdir,genome,chrN,outdir):  
    O_overlap=merge_columns_in_bed_file(os.path.join(outdir, f'Region_overlap_{chrN}.bed'),1)
    N_overlap=merge_columns_in_bed_file(os.path.join(outdir, f'Region_overlap_{chrN}.bed'),4)
...
def load_TF_RE(GRNdir,chrN,O_overlap,O_overlap_u,O_overlap_hg19_u):      
    #logging.info('load prior TF-RE for '+chrN+'...')
    mat=pd.read_csv(os.path.join(GRNdir, f'Primary_TF_RE_{chrN}.txt'),sep='\t',index_col=0)
...
def TF_RE_LINGER_chr(chr,outdir):
    REName = os.path.join(outdir, 'Peaks.txt')
# Open the file in read mode
    ...
    net_all=torch.load(os.path.join(outdir, f"net_{chr}.pt"))
    data_merge=pd.read_csv(os.path.join(outdir, 'data_merge.txt'),sep='\t',header=0,index_col=0)
...
def load_data_scNN(GRNdir,data_dir,genome, output_dir):
    ...
    RE_TGlink=pd.read_csv(os.path.join(output_dir, 'RE_gene_distance.txt'),sep='\t',header=0)
...
def cis_shap(chr,outdir):
    ...
    for j in range(data_merge_temp.shape[0]):
...
def load_RE_TG(GRNdir,chrN,O_overlap_u,O_overlap_hg19_u,O_overlap):   
    #logging.info('load prior RE-TG ...')
    primary_s=pd.read_csv(os.path.join(GRNdir, f'Primary_RE_TG_{chrN}.txt'),sep='\t')
...
def load_RE_TG_distance(GRNdir,chrN,O_overlap_hg19_u,O_overlap_u,O_overlap,TGoverlap):
    Dis=pd.read_csv(os.path.join(GRNdir, f'RE_TG_distance_{chrN}.txt'),sep='\t',header=None)
...
def load_RE_TG_scNN(outdir):   
    dis=pd.read_csv(os.path.join(outdir, 'RE_gene_distance.txt'),sep='\t',header=0)
...
def load_TF_TG( GRNdir, TFset,TGset):
    ...
    for i in a:
        chrN='chr'+str(i)     
        TF_TG = pd.read_csv(os.path.join(GRNdir, f'Primary_TF_TG_{chrN}.txt'),sep='\t')
